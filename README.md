...
* [`src/girouette_issue_90/`](src/girouette_issue_90/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/girouette_issue_90/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `girouette-issue-90` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/girouette_issue_90/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns girouette-issue-90.example
  (:require [girouette-issue-90.config :as config])
